...
namespace KineticEnergy.Grids {
    ...
    public class BlockGrid : MonoBehaviour, IEnumerable {
        ...
        /// Instead, the <see cref="Block"/> script updates it on <see cref="Block.SetGridAndPosition"/> and when 'set'-ing <see cref="Block.Mass"/>.</remarks>
        public Mass Mass {
            get => mass;
            internal set {
                mass = value;
        ...
        /// <summary>Represents collections of event listeners.</summary>
        public class EventActions {
            /// <summary>A collection of <see cref="Action"/>s that are called when a block is removed.</summary>
        ...
        /// <remarks>While <see cref="SideIsOpaque(Vector3Int, Block.Face)"/> checks the block at the given position,
        /// this function checks the block touching the given position.</remarks>
        /// <seealso cref="SideIsOpaque(Vector3Int, Block.Face)"/>
        ...
        /// <summary><see cref="Vector3"/> shortcut of <see cref="SideIsCovered(Vector3Int, Block.Face)"/>.</summary>
        ...
        /// <param name="gridPosition">Position of the side to check.</param>
        /// <param name="relativeFace">The face of the given position to check.</param>
        /// <remarks>While <see cref="SideIsCovered(Vector3Int, Block.Face)"/> checks the block touching the given position,
        ...
        /// <summary><see cref="Vector3"/> shortcut of <see cref="SideIsOpaque(Vector3Int, Block.Face)"/>.</summary>
        ...
        public void PlaceBlock(Block block, Vector3Int gridPosition) {
            ...
            foreach(Vector3Int relativePoint in block.NeighborPoints) {
                Vector3Int neighboringGridPoint = relativePoint + gridPosition;
                Block neighbor = TryGetBlockAt(neighboringGridPoint);
                if(neighbor != null && neighbor != block) //As long as somebody's home... && who isn't you.
                    neighbor.OnNearbyPieceAdded(gridPosition - neighbor.Location.GridVector);
            }

            //Tell the block that we're done and that it's <<here>>.
            block.Location = new GridLocation(this, gridPosition);
        ...
        /// <summary>Instantiates a prefab, defines important values such as transform data and name then calls <see cref="Block.UpdatePoints()"/>,
        ...
        /// <param name="gridPosition">Grid position to place the block at.</param>
        /// <param name="localRotation">Rotation of the new <see cref="Block"/> relative to this <see cref="BlockGrid"/>.</param>
        /// <remarks>Subject to clipping if <see cref="CanPlaceBlock(Block, Vector3Int)"/> is false.</remarks>
        public Block PlaceNewBlock(Prefab<Block> prefab, Vector3Int gridPosition, Quaternion localRotation) {

            Instantiated<Block> block = prefab.Instantiate(transform, false);
            block.gameObject.transform.localRotation = localRotation;
            Global.PolishBehaviour(block.component);
        ...
        public Block RemoveBlock(Vector3Int gridPosition) {

            if(!Master) {
                //Debug.LogFormat("Ignored {0} because there was no master.", nameof(RemoveBlock));
                return null;
            }
            ...
            if(Separations(gridPosition, out LinkedList<Traversal> traversals) > 1) {
                ...
                foreach(Traversal traversal in traversals) {
                    if(traversal.Root.Element && !traversal.IsActive) {
                        ...
                        //Create the new grid.
                        Prefab<BlockGrid> prefab = Master.prefabs.grid;
                        ...
                        foreach(Block element in removal) {
                            RemoveBlockReferences(element);
                            grid.component.PlaceBlock(element, element.Location.GridVector);
                        }
        ...
        /// <summary>Removes the given <see cref="Block"/> from <see cref="Array"/> by using <see cref="Block.ArrayPosition"/>.</summary>
        /// <param name="block">The <see cref="Block"/> to remove.</param>
        /// <param name="setGrid">Set <see cref="Block.Grid"/> to null?</param>
        private void RemoveBlockReferences(Block block, bool setGrid = true) {

            GridLocation location = block.Location;
            Vector3Int gridPosition = location.GridVector;
            Vector3Int arrayPosition = location.ArrayVector;

            //Remove all refrences to this block from the array.
            foreach(Vector3Int relativePosition in block.InsidePoints) {
                var insideArrayPosition = relativePosition + arrayPosition;
                this[insideArrayPosition] = null;
            }

            //call neighbor.OnNerbyPieceRemoved()
            foreach(Vector3Int relativePosition in block.InsidePoints) {
                var neighboringGridPosition = relativePosition + gridPosition;
                foreach(Block neighbor in GetNeighborsAt(neighboringGridPosition)) {
                    if(neighbor != null && neighbor != block) //As long as somebody's home... && who isn't you.
                        neighbor.OnNearbyPieceRemoved(neighboringGridPosition - neighbor.Location.GridVector);
                }
            }
        ...
        /// <param name="final">A collection of non-redundant <see cref="Traversal"/>s that describe the found separations.</param>
        /// <returns>Returns the number of grids after a separation at the given grid position.</returns>
        public int Separations(Vector3Int position, out LinkedList<Traversal> final) {
            ...
            if(CanShowLog(LevelOfDetail.Basic, Master?.LogSettings.traversal ?? LevelOfDetail.None))
                Debug.LogFormat("<2> Returned Group Size: {0}", final.Count);
            #endregion
        ...
        /// <summary>Represents a group of <see cref="Traversal"/>s that are traversing together.</summary>
        public class TraversalGroup {

            /// <summary>The <see cref="Traversal"/>s in this <see cref="TraversalGroup"/>.</summary>
            public IEnumerable<Traversal> Traversals { get; }
            /// <summary>Identifier used to check if a <see cref="Traversal.Node"/> has already been iterated over.</summary>
            public long Seed { get; }

            /// <summary>Contructor for a new <see cref="TraversalGroup"/>.</summary>
            /// <param name="traversals">The <see cref="Traversal"/>s to put into this <see cref="TraversalGroup"/>.</param>
            /// <param name="seed">Identifier used to check if a <see cref="Traversal.Node"/> has already been iterated over.</param>
            public TraversalGroup(IEnumerable<Traversal> traversals, long seed) {
                Traversals = traversals;
                Seed = seed;
            }

            /// <summary>Calls <see cref="Traversal.StartEnumeration(TraversalGroup)"/> and puts <see cref="Traversals"/> into a <see cref="Queue"/>.</summary>
            /// <returns>Returns a new <see cref="Queue"/> from <see cref="Traversals"/>.</returns>
            ...
            /// <summary>The number of <see cref="Traversal"/>s in <see cref="Traversals"/> with <see cref="Traversal.IsActive"/> true.</summary>
            ...
            /// <summary>A collection of all non-redundant <see cref="Traversal.Root"/>s.</summary>
            ...
            /// <summary>Combines the two given <see cref="Traversal"/>s.</summary>
            ...
            /// <summary>Returns a collection of non-redundant <see cref="Traversal"/>s.</summary>
        ...
        public class Traversal : IEnumerable<Traversal.Node> {
            ...
            /// <summary>Implementation of <see cref="IEnumerable{Node}.GetEnumerator()"/></summary>
            ...
            public class ConnectionEnumerator : IEnumerator<Node> {
                ...
                /// <summary>Updates what should be logged.</summary>
        ...
        /// <returns>Returns the <see cref="IEnumerator"/> of <see cref="Array"/>.</returns>
        public BoxOfPoints ArrayPoints => new Enumeration.BoxOfPoints(Size);
